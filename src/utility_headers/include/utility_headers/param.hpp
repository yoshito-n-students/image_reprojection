...
namespace utility_headers{
  namespace param{
	...
	if(src.getType() != XrvType::TypeBoolean){ return false; }
    ...
    struct Helper<std::string>{
      ...
      static bool cast(const XrvType & src,ValueType & dst){
	if(src.getType() != XrvType::TypeString){ return false; }
    ...
    struct Helper<std::vector<T,A> >{
      ...
      static bool cast(const XrvType & src,ValueType & dst){
	if(src.getType() != XrvType::TypeArray){ return false; }
	dst.resize(src.size());
	for(std::size_t i=0;i<src.size();++i){
	  if(!Helper<T>::cast(src[i],dst[i])){ return false; }
	}
	return true;
      }
	...
	for(std::size_t i=0;i<src.size();++i){
      ...
      static bool get(const std::string & name,ValueType & val){
	XrvType xrv;
	if(!ros::param::get(name,xrv)){ return false; }
	if(!ThisType::cast(xrv,val)){ return false; }
	return true;
      }
    ...
    struct Helper<std::map<std::string,T,C,A> >{
      ...
      static bool cast(const XrvType & src,ValueType & dst){
	if(src.getType() != XrvType::TypeStruct){ return false; }
	dst.clear();
	for(XrvType::ValueStruct::const_iterator it=const_cast<XrvType &>(src).begin();
	  ...
	  if(!Helper<T>::cast(it->second,elm.second)){ return false; }
	  if(!dst.insert(elm).second){ return false; }
	}
	return true;
      }
    ...
    template<typename T>
    T param(const ros::NodeHandle & handle,
    ...
    template<typename T>
    bool get(const ros::NodeHandle & handle,
	     const std::string & name,
	     T & val,
	     const bool do_print = true){
    ...
    class NoRequiredParameterException:
    ...
    template<typename T>
    void getRequired(const std::string & name,
		     T & val,
		     const bool do_print = true){
    ...
    template<typename T>
    void getRequired(const ros::NodeHandle & handle,
		     const std::string & name,
		     T & val,
		     const bool do_print = true){
    ...
    static inline bool del(const std::string & name){
      return ros::param::del(name);
    }
