...
class VerboseFilter : public ros::console::FilterBase {
   private:
    VerboseFilter()
   ...
   public:
    ...
    virtual bool isEnabled(ros::console::FilterParams &param) {
        ...
        if (show_function || show_file) {
            ...
            if (show_file) {
                oss << "@" << bf::path(param.file).filename().string() << ":" << param.line;
            }
            oss << ")";
        }
   ...
   public:
    ...
    static void showNode(const ros::console::Level &level) { ptr()->level_node_ = level; }

    static void showFunction(const ros::console::Level &level) { ptr()->level_function_ = level; }

    static void showFile(const ros::console::Level &level) { ptr()->level_file_ = level; }
