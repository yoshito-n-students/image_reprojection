...
namespace image_reprojection_plugins{
  ...
  class FisheyeProjectionPlugin:
    public image_reprojection::ProjectionPlugin{
  public:
  ...
  private:
    ...
    virtual void onProject(const cv::Mat & src,cv::Mat & dst,cv::Mat & mask){
      cv::fisheye::projectPoints(src.reshape(3,src.total()),dst,
				 cv::Vec3d::all(0.),cv::Vec3d::all(0.),
    ...
    virtual void onReproject(const cv::Mat & src,cv::Mat & dst,cv::Mat & mask){
      cv::fisheye::undistortPointsTo3D(src.reshape(2,src.total()),dst,
				       camera_matrix_,dist_coeffs_);
